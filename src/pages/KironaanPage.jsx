...
import KironaanComposer from '../components/KironaanComposer';
import KironaanGrammarChecker from '../components/KironaanGrammarChecker';
...
const card = { width: '100%', maxWidth: 700, padding: 24, background: 'white', borderRadius: 8, boxShadow: '0 4px 8px rgba(0,0,0,0.1)', marginBottom: 24, boxSizing: 'border-box' };
...
function EditModal({ target, terms, onSave, onClose }) {
    ...
    return (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ background: 'white', borderRadius: 10, padding: 28, minWidth: 320, maxWidth: 440, width: '90%', boxShadow: '0 10px 40px rgba(0,0,0,0.25)', maxHeight: '90vh', overflowY: 'auto' }}>
                <h3 style={{ marginTop: 0, marginBottom: 18 }}>Edit Term</h3>

                {/* English definitions tag input */}
                        ...
                        {translations.map((v, i) => {
                            ...
                            return (
                                <span key={i} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '2px 8px', borderRadius: 12, fontSize: 13, background: dup ? '#fff3e0' : '#e3f2fd', border: `1px solid ${dup ? '#ff9800' : '#90caf9'}` }}>
                                    ...
                                    <button onClick={e => { e.stopPropagation(); setTranslations(p => p.filter((_, j) => j !== i)); }}
                ...
                {/* Kironaan term — note: no consonant doubling suggestion here (Composer only) */}
                <div style={{ marginBottom: 12 }}>
                ...
                <div style={{ marginBottom: 20 }}>
                    <label style={{ display: 'block', fontSize: 13, marginBottom: 4, color: '#555' }}>
                        Type
                        {detectedType && <span style={{ marginLeft: 6, fontSize: 12, padding: '2px 7px', borderRadius: 10, background: '#e3f2fd', color: '#1565c0' }}>NLP: {detectedType}</span>}
                        {type !== detectedType && (
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                    <button onClick={save} style={{ flex: 1, padding: 10, background: '#4caf50', color: 'white', border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: 15 }}>Save</button>
                    <button onClick={onClose} style={{ flex: 1, padding: 10, background: '#eee', border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: 15 }}>Cancel</button>
                </div>
            </div>
        </div>
    );
...
function KironaanPage() {
    ...
    useEffect(() => { getAllKironaanTerms().then(setTerms).finally(() => setLoading(false)); }, []);
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minHeight: '100vh', background: '#f9f9f9', paddingTop: 20 }}>
            <div style={{ ...card, marginTop: 30 }}>
                <KironaanComposer userId={userId} terms={terms} onTermsChange={setTerms} />
            </div>
            <div style={card}>
                <KironaanGrammarChecker terms={terms} />
            </div>
            ...
            <div style={card}>
                ...
                <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
                    <input type="text" placeholder="Search term or translation..." value={search}
                        ...
                        style={{ flex: 1, minWidth: 180, padding: '8px 10px', border: '1px solid #ccc', borderRadius: 5, fontSize: 15 }} />
                    <select value={typeFilter} onChange={e => setTypeFilter(e.target.value)}
                        style={{ padding: '8px 10px', border: '1px solid #ccc', borderRadius: 5, fontSize: 15, minWidth: 140 }}>
                ...
                {loading ? (
                    ...
                    <>
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 15 }}>
                            ...
                            <tbody>
                                {paginated.map((item, i) => (
                                    <tr key={item.id} style={{ background: i % 2 === 0 ? 'white' : '#fafafa', borderBottom: '1px solid #eee' }}>
                                        <td style={td}>
                                            <strong className="KironaanFont" style={{ fontSize: 13 }}>{item.term}</strong>
                                            <span style={{ display: 'block', fontSize: 12, color: '#888', marginTop: 1 }}>{decodeKironaan(item.term)}</span>
                                        </td>
                                        <td style={td}>{item.translation}</td>
                                        <td style={{ ...td, color: '#888', fontSize: 13 }}>{item.type}</td>
                                        {isAdmin && (
                                            <td style={td}>
                                                <button onClick={() => setEditTarget(item)} style={actionBtn('#1976d2')}>Edit</button>
                                                <button onClick={() => handleDelete(item.id)} style={actionBtn('#e53935')}>Del</button>
                                            </td>
                                        )}
                                    </tr>
                                ))}
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 14, flexWrap: 'wrap', gap: 8 }}>
                            <span style={{ fontSize: 13, color: '#888' }}>{filtered.length} results — page {page} of {totalPages}</span>
            ...
            {editTarget && (
                <EditModal target={editTarget} terms={terms} onSave={handleSave} onClose={() => setEditTarget(null)} />
            )}
        </div>
    );
}
...
const actionBtn = (color) => ({ padding: '2px 8px', fontSize: 12, border: `1px solid ${color}`, color, background: 'white', borderRadius: 4, cursor: 'pointer', marginRight: 4 });
const pageBtn   = (dis) => ({ padding: '4px 10px', border: '1px solid #ccc', borderRadius: 4, cursor: dis ? 'default' : 'pointer', background: dis ? '#f5f5f5' : 'white', color: dis ? '#bbb' : '#333', fontSize: 14 });
