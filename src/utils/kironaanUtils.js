...
// Floating punctuation - must have spaces around them
...
export const NUMBER_TERMS = {
    'xás': 0, 'brás': 1, 'derás': 2, 'qirás': 3, 'esán': 4,
    'derán': 5, 'sīkán': 6, 'xekán': 7, 'asī': 8, 'asībrax': 9,
    'pósárasī': 16, 'phosáderáquinasī': 24, 'derásī': 32
};
...
export const parseSigils = (tok) => {
    // Don't parse floating punctuation
    if (FLOATING_PUNCT.includes(tok)) {
        return {
            base: tok,
            prefixes: [],
            hasPossession: false,
            leadingPunct: '',
            trailingPunct: '',
            isConceptShortcut: false,
            isFloatingPunct: true
        };
    }

    // Check if it's a standalone concept shortcut
    if (tok.length === 1 && isConceptShortcut(tok)) {
        return {
            base: tok,
            prefixes: [],
            hasPossession: false,
...
export const buildCompound = (baseTerm, prefixes, hasPossession, adverbTerm = null, leadingPunct = '', trailingPunct = '') => {
    ...
    if (adverbTerm) {
        // Check if adverb is actually a verb (compound verb construction)
        if (adverbTerm.endsWith('mir')) {
            // Convert baseVerbmir to baseVerbmī for compound
            parts.push(adverbTerm.replace(/mir$/, 'mī'));
        } else {
            parts.push(adverbTerm);
        }
    }

    if (hasPossession) parts.push('orsik');

    return `${leadingPunct}${parts.join('_')}${trailingPunct}`;
};

export const splitKironaanTokens = (text) => {
    // Split by spaces first, preserving them
    const tokens = text.split(/(\s+)/);

    return tokens.map(tok => {
        if (/^\s+$/.test(tok)) return tok; // whitespace
        if (FLOATING_PUNCT.includes(tok)) return tok; // floating punctuation
        return tok;
    });
};

export const computeTokenMeta = (tokens, terms) => {
    const wordIndices = tokens.map((tok, i) => {
        if (/^\s+$/.test(tok)) return null;
        if (FLOATING_PUNCT.includes(tok)) return null;
        return i;
    }).filter(i => i !== null);
    ...
    for (let wi = 0; wi < wordIndices.length; wi++) {
        ...
        const sortedPhrases = [...phraseTerms].sort((a, b) =>
            b.translation.split(/\s+/).length - a.translation.split(/\s+/).length
        );
...
export const findPartialPhraseMatches = (tokens, terms) => {
    const wordTokens = tokens.filter(t => {
        ...
        if (FLOATING_PUNCT.includes(t)) return false;
        return true;
    });
    ...
    return terms
        ...
        .flatMap(pt => {
            ...
            for (const meaning of meanings) {
                ...
                const apply = (text) => {
                    const toks = splitKironaanTokens(text);
...
export const analyzeGrammarSuggestions = (text) => {
    ...
    doc.verbs().forEach(phrase => {
        ...
        if (phrase.has('#PastTense')) {
            ...
            if (base && base !== verbText.toLowerCase()) {
                suggestions.push({
                    id: `past-${verbText}`,
                    message: `"${verbText}" is past tense → pó_${base}`,
                    type: 'tense',
                    apply: (t) => t.replace(new RegExp(`(?<![?*^#&!])\\b${verbText}\\b`, 'gi'), `pó_${base}`)
                });
            }
        }
    });
    ...
    if (willMatch) {
        ...
        if (!rawToken || !/^[?*^#&!]/.test(rawToken)) {
            suggestions.push({
                ...
                message: `"will ${futureVerb}" is future tense → tó_${futureVerb}, drop "will"`,
                type: 'tense',
                apply: (t) => t.replace(new RegExp(`\\bwill\\s+${futureVerb}\\b`, 'gi'), `tó_${futureVerb}`)
            });
        }
    }
    ...
    if (trimmed.endsWith('?')) {
        ...
        if (targetVerbBase) {
            suggestions.push({
                id: 'question-mark',
                message: `"?" at end — move as question prefix onto "${targetVerbBase}"`,
                type: 'punctuation',
                apply: (t) => {
                    ...
                    const replaced = result.replace(
                        new RegExp(`([pó_tó_]?)\\b${escaped}\\b`, 'i'),
                        (_, prefix) => `sók_${prefix}${targetVerbBase}`
                    );
                    return replaced === result ? `sók_${result}` : replaced;
                }
            });
        }
    }
    ...
    wordIndices.forEach((tokenIdx, wordIdx) => {
        ...
        if (type === 'Pronouns' && lastVerbIdx > -1 && lastVerbIdx < tokenIdx) {
            violations.push({
                ...
                apply: (text) => {
                    const parts = splitKironaanTokens(text);
        ...
        if (type === 'Adjective' && lastNounIdx > -1 && lastNounIdx < tokenIdx) {
            violations.push({
                ...
                message: `"${base}": adjective after noun — compound as descriptor_noun`,
                type: 'word-order',
                word: base,
                ...
                apply: (text) => {
                    ...
                    const result = [...parts];
                    result[nounIdx] = `${result[adjIdx]}_${result[nounIdx]}`;
                    result[adjIdx] = '';
...
export const buildKironaanNumber = (decimal) => {
    ...
    if (pow >= 4) {
        ...
        if (base < 8) {
            ...
            if (baseTerm && powTerm) {
                parts.push(`${baseTerm}_ann_${powTerm}`);
    ...
    if (remaining >= 32) {
        ...
        if (count === 1) parts.push('derásī');
            ...
            if (countTerm) parts.push(`${countTerm}_derásī`);
        }
        remaining %= 32;
    }
    ...
    if (remaining >= 8) {
        ...
        if (count === 1) parts.push('asī');
            ...
            const countTerm = Object.keys(NUMBER_TERMS).find(k => NUMBER_TERMS[k] === count);
            if (countTerm) parts.push(`${countTerm}_asī`);
        }
        remaining %= 8;
    }
...
export const parseKironaanNumber = (kironaanStr) => {
    const segments = kironaanStr.toLowerCase().split('_').map(s => s.trim());
    let total = 0;
    let current = null;
    let nextOp = null;
